...
SYSTEM_PROMPT = (
    ...
    'Return JSON of the form {"propositions": [{"text": "..."}]}.'
)
...
class PropositionResult:
    """One extracted proposition with its provenance and dense vector."""
...
class Propositionizer:
    ...
    async def _infer_span(
        ...
        texts = [p.text for p in extraction.propositions]
        if not texts:
            return []

        # Sync torch forward pass — run off the event loop so concurrent LLM calls flow.
        vectors = await asyncio.to_thread(self.substrate.embed_passages, texts)
        return [
            PropositionResult(
                id=uuid.uuid4(),
                text=t,
    ...
    async def _persist(
        ...
        for r in results:
            await execute_cypher(
                session,
                f"CREATE (p:Proposition {cypher_map({'id': str(r.id), 'text': r.text})}) RETURN p",
                returns="p agtype",
            )
            await execute_cypher(
                session,
