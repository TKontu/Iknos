...
1. `types/epistemic.py`: add `ProvisionalReason(StrEnum)` with those four values;
   ...
   bool wrapper. **Merge-order note (2026-06-11):** R8 shipped (via #72) with the
   original members and `None → set()` — written before this amendment landed.
   **G1.21 (shipped, `todo_phase_1_ingest.md`) then delivered the
   `UNASSESSED_FAITHFULNESS` member and the `None → {UNASSESSED_FAITHFULNESS}`
   mapping** as the follow-up, so the quarantine gate now *does* hold back unverified
   (verifier-off) propositions — the §3.1 D2 rule is code-complete.
